#####
# to use, see folder tree example in ../README.md
#####
...
# args
ref     = sys.argv[1] # path to reference genome used for mapping (/home/lindb/scratch/ptaeda.v1.01.reduced.pseudo.fasta)
fqdir   = sys.argv[2] # path to raw fastq files
...
pipedir = op.join(op.dirname(sys.argv[0]),'pipeline')         # this is where the git repo (pipeline) is pulled
try:
    imp = open(os.path.join(pipedir,'pythonimports.py')).read()
except IOError as e:
...
text = 'found %s gz files in %s' % (lgz, fqdir)
...
#     print s
    r1    = op.abspath(s[0])
    r1out = op.join(trimDIR,op.basename(r1).replace('.fastq.gz','_trimmedandfiltered.fastq'))
    r2    = op.abspath(s[1])
    r2out = op.join(trimDIR,op.basename(r2).replace('.fastq.gz','_trimmedandfiltered.fastq'))
    cmd   = '''fastp -i %s -o %s \           
-I %s -O %s \   
\
-g \
